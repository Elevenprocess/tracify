// @ts-nocheck
/* eslint-disable */
// Composant « Hyperspeed » de React Bits (reactbits.dev) — code vendor conservé tel quel.
import {
  BloomEffect,
  EffectComposer,
  EffectPass,
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    // Si WebGL est indisponible, on laisse simplement le fond noir plutôt que de crasher la page
    let myApp
    try {
      myApp = new App(container, options)
    } catch (e) {
      console.warn('Hyperspeed désactivé :', e)
      return
    }
